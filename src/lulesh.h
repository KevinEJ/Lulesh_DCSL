...
public: 
  ...
  Vector_d<Real_t_x> xd ;           /* velocities */
  Vector_d<Real_t_x> yd ;
  Vector_d<Real_t_x> zd ;
  ...
  Vector_d<Real_t_x> fx ;           /* forces */
  Vector_d<Real_t_x> fy ;
  Vector_d<Real_t_x> fz ;

  Vector_d<Real_t_x> nodalMass ;    /* mass */
  Vector_h<Real_t_x> h_nodalMass ;    /* mass - host */

  /* device pointers for comms */
  Real_t_delv_xi *d_delv_xi ;      /* velocity gradient -- temporary */
  Real_t_delv_eta *d_delv_eta ;
  Real_t_delv_zeta *d_delv_zeta ;

  Real_t_x *d_x ;            /* coordinates */
  Real_t_x *d_y ;
  Real_t_x *d_z ;

  Real_t_x *d_xd ;           /* velocities */
  Real_t_x *d_yd ;
  Real_t_x *d_zd ;

  Real_t_x *d_fx ;           /* forces */
  Real_t_x *d_fy ;
  Real_t_x *d_fz ;

  /* access elements for comms */
  Real_t_delv_xi& get_delv_xi(Index_t idx) { return d_delv_xi[idx] ; }
  Real_t_delv_eta& get_delv_eta(Index_t idx) { return d_delv_eta[idx] ; }
  Real_t_delv_zeta& get_delv_zeta(Index_t idx) { return d_delv_zeta[idx] ; }

  Real_t_x& get_x(Index_t idx) { return d_x[idx] ; }
  Real_t_x& get_y(Index_t idx) { return d_y[idx] ; }
  Real_t_x& get_z(Index_t idx) { return d_z[idx] ; }

  Real_t_x& get_xd(Index_t idx) { return d_xd[idx] ; }
  Real_t_x& get_yd(Index_t idx) { return d_yd[idx] ; }
  Real_t_x& get_zd(Index_t idx) { return d_zd[idx] ; }

  Real_t_x& get_fx(Index_t idx) { return d_fx[idx] ; }
  Real_t_x& get_fy(Index_t idx) { return d_fy[idx] ; }
  Real_t_x& get_fz(Index_t idx) { return d_fz[idx] ; }

  // host access
  Real_t_x& get_nodalMass(Index_t idx) { return h_nodalMass[idx] ; }
  ...
  Index_t octantCorner;
   ...
   void BuildMesh(Int_t nx, Int_t edgeNodes, Int_t edgeElems, Int_t domNodes, Int_t padded_domElems, Vector_h<Real_t_x> &x_h, Vector_h<Real_t_x> &y_h, Vector_h<Real_t_x> &z_h, Vector_h<Int_t> &nodelist_h);
...
typedef Real_t_x& (Domain::* Domain_member )(Index_t) ;
